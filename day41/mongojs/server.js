...
const getGroceries = () => {
  ...
  db.collection('groceries').find((err, groceries) => {
    if (err) throw err

    console.log('Grocery List:')
    groceries.forEach(grocery => console.log(grocery.name))
...
const addGroceries = () => {
  prompt({
    ...
    .then(({ grocery }) => {
      ...
      db.collection('groceries').insert({ name: grocery }, err => {
        if (err) throw err
...
const removeGroceries = () => {
  prompt({
    ...
    .then(({ grocery }) => {
      ...
      db.collection('groceries').remove({ name: grocery }, err => {
        if (err) throw err
